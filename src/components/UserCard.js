...
export default function UserCard({ user, onBookmark, onPromote }) {
  ...
  return (
    ...
    <div style={{ backgroundImage: `url(${user.image})` }} className="absolute inset-0  bg-cover bg-center bg- opacity-0 group-hover:opacity-20 transition-opacity duration-300 z-0"></div>
      ...
      <h1 className="text-2xl text-gray-800 text-center hover:text-lg font-black group-hover:text-black">{user.fullName}</h1>
        ...
        <div className="flex mt-4 md:mt-6">
          {['View', 'Bookmark', 'Promote'].map((label, i) => (
            <button
              key={label}
              onClick={() => label === 'Promote' ? onPromote(user) : onBookmark(user)}
